...
const ViewReports = () => {
    ...
    useEffect(() => {
        const fetchPendingOrders = async () => {
            try {
                const data = await (await getFirestore().collection('orders').get()).docs
    ...
    return (
        <View style={styles.container}>
            ...
            renderItem={({ item }) => {
                console.log(item)
                return (
                        <TouchableOpacity style={styles.cartItem} onPress={() => navigation.navigate('PendingOrders',{ item:item.data.OrdersPending })}>
